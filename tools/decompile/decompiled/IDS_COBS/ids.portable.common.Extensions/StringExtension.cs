...
public static class StringExtension
{
	...
	public static string Truncate(this string value, int maxLength, string truncationAppend = "")
	{
		...
		int num = maxLength - truncationAppend.Length;
		return value.Substring(0, num) + truncationAppend;
	}

	public static string TrimStartString(this string inputText, string value, StringComparison comparisonType = (StringComparison)1)
	{
	...
	public static string TrimEndString(this string inputText, string value, StringComparison comparisonType = (StringComparison)1)
	{
	...
	public static string TrimString(this string inputText, string value, StringComparison comparisonType = (StringComparison)1)
	{
	...
	{
		if (intAsString.StartsWith("0x", (StringComparison)1))
		{
			try
			{
				int num = Convert.ToInt32(intAsString.Remove(0, 2), 16);
				result = num;
				return true;
			}
		...
		if (intAsString.StartsWith("0b", (StringComparison)1))
		{
			try
			{
				int num2 = Convert.ToInt32(intAsString.Remove(0, 2), 2);
				result = num2;
				return true;
			}
		...
		return int.TryParse(intAsString, ref result);
	}
